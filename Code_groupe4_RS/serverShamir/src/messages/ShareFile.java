...
public class ShareFile {
	...
	public static void shareFile(String fileGID, int userGID, User myInfo, String source, String privKey){
		...
		try {
			...
			MyInfo user = TcellDAO.getInstance().getUserByID(userGID);
			...
			Socket socket = new Socket(tcellIP, port);
			/* Creation of the stream */
			IOStreams stream = new IOStreams(socket);
			
			/* send the share command to the recipient TCELL */
			Command shareFileCommand = new ShareFileCommand(Constants.CMD_SHARE_FILE, fileToShare, source, privKey);
